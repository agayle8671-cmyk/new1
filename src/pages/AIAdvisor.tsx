...
export default function AIAdvisor() {
  ...
  const getContext = useCallback(async (): Promise<AIContext> => {
    ...
    // Extract data from simulator params (only if explicitly set, not default values)
    // Default simulator values: cashOnHand=500000, monthlyExpenses=50000, monthlyRevenue=25000
    const isSimulatorCustomized = simulatorParams && (
      simulatorParams.cashOnHand !== 500000 ||
      simulatorParams.monthlyExpenses !== 50000 ||
      simulatorParams.monthlyRevenue !== 25000
    );

    if (isSimulatorCustomized) {
      // User has customized simulator - use those values as more current
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      // Filter out welcome message from history (it's static, not real conversation)
      const conversationHistory = messages.filter(m => m.id !== 'welcome');
      const response = await AIService.chat(userQuestion, context, conversationHistory);
            ...
            <div className="space-y-3">
              {insights.map((insight, i) => (
                <motion.div
                  ...
                  className={`p-3 rounded-lg border ${insight.type === 'success' ? 'border-success/30 bg-success/5' :
                      ...
                      <div className="flex flex-wrap gap-3 mt-3">
                        {section.metrics.map((m, j) => (
                          <div key={j} className="bg-white/5 px-3 py-1.5 rounded-lg">
                            ...
                            <span className={`text-sm font-semibold ${m.trend === 'up' ? 'text-success' : m.trend === 'down' ? 'text-danger' : 'text-white'
                              }`}>
                              {m.value}
                            </span>
                          </div>
